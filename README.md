...
> [!TIP]
> You can query the AI docs (see [Docs](#docs)) to get coding examples, and understand Py-V better!
...
- `csr.py`: Contains a RISC-V CSR (_control and status registers_) module
- `exception_unit.py`: Contains an exception unit to handle various RISC-V exceptions
...
- `reg.py`: Contains definitions for registers
  - Also defines a RISC-V register file
- `simulator.py`: Contains the main simulator logic
- `stages.py`: Module definitions for the various pipeline stages
- `test_utils.py`: Contains utilities for tests
...
Help is always welcome! If you have ideas, bugs, feature requests, etc., feel free to open an issue.

If you want to implement code yourself, here are the steps to get you started:

1. Clone the repo to your local machine
2. Navigate to the local repo copy, and install the requirements:
...
### Note on using AI coding agents

If you open a PR where the majority of code changes were generated by AI (e.g. you gave it a prompt like "Implement an MMU" and let it handle the design and coding), I would highly appreciate it if you could clearly mention that in the description. To be clear: I'm _not_ against using AI agents (I use them often myself), but I'd like to understand how much of the code is human-written versus AI-generated. As a matter of fact, Py-V was created almost entirely in the "pre-ChatGPT era", and I prefer not using AI on this project so I can keep sharpening my own coding skills. That said, I'm not saying _you_ shouldn't use AI — just please be transparent about it :)
